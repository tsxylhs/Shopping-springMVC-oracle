...
--插入测试数据权限
    insert into permission values(1,'会员列表','vipmanager','对会员操作');
    insert into permission values(1,'商品列表','goodslist','对商品操作');
    insert into permission values(1,'添加新商品','addgoods','添加商品');
    insert into permission values(1,'角色权限','rolepermission','给角色添加权限');
    insert into permission values(1,'用户角色','userrole','用户添加角色');
    insert into permission values(1,'用户列表','findusers','显示所有用户');
     insert into permission values(1,'权限列表','findpri','显示角色权限');
      insert into permission values(1,'角色列表','findrole','显示所有角色');
...
alter table user_role add(constraint USER_ROLE_f foreign key(role_id) references role(id));
    ...
    create or replace trigger t_users_role
      ...
      select * from users
       ...
       commit
       delete  user_role  where userId=10002;
        --查询测试数据
 ...
 alter table role_permission add(constraint role_permision_f1 foreign key(permission_id) references permission(id));
   ...
   --给角色添加权限
    insert into role_permission  values(1,10002,10003);
    insert into role_permission  values(1,10002,10002);
    insert into role_permission  values(1,10001,10003);
    insert into role_permission  values(1,10001,10001);
   insert into role_permission  values(1,10001,10004);
   insert into role_permission  values(1,10001,10005);
   insert into role_permission  values(1,10001,10021);
    insert into role_permission  values(1,10001,10022);
    insert into role_permission  values(1,10001,10023);
    select * from permission;
   select * from role_permission 
   select * from user_role
   select * from role;
   select * from users;
   delete user_role where user_Id=10002
   delete  role_permission  where permission_id=10026;
   delete from users where id=10064
  ...
  commit
    --根据用户ID 查询其权限
    select u.username, p.id,p.per_name,p.per_url,p.per_massger from Users u inner join user_role ur on u.id=ur.user_id inner join role_permission rp on ur.role_id=rp.role_id inner join permission p on rp.permission_id=p.id where u.id=10002;
    ...
    commit
    select * from role
     select  rp.id,r.rolename,p.per_name,p.per_massger from role r inner join role_permission rp on r.id=rp.role_id inner join permission p on rp.permission_id=p.id;
    select  sr.id, u.username, r.rolename,r.massage  from users u inner join user_role sr on u.id=sr.user_id inner join role r on sr.role_id=r.id;
            /*
        oracle 功能部分
        */
   ...
   procedure Users_role_permission_init
     ...
     insert into user_role values(1,10001,10001);
       ...
       insert into role_permission  values(1,10002,10001);
        ...
        insert into role_permission  values(1,10001,10004);
          insert into role_permission  values(1,10001,10005);
            insert into role_permission  values(1,10001,10006);
              insert into role_permission  values(1,10001,10007);
